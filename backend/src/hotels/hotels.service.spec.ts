...
describe('HotelsService (state machine)', () => {
    ...
    beforeAll(async () => {
        ...
        await ds.getRepository(User).save([
            { id: MERCHANT_ID, username: 'merchant1', password: 'x', role: UserRole.MERCHANT } as any,
            { id: OTHER_MERCHANT_ID, username: 'merchant2', password: 'x', role: UserRole.MERCHANT } as any,
        ]);
    });
    ...
    // ============ 辅助函数 ============
    async function createHotelInStatus(status: HotelStatus, merchantId = MERCHANT_ID): Promise<Hotel> {
            ...
            const hotel = await service.create(
                {
                    ...
                    roomTypes: [{ name: '豪华大床房', price: 888, maxGuests: 2, bedType: '2.0m大床' }],
                    images: [{ imageUrl: 'https://example.com/img.jpg', description: '外观' }],
                } as any,
                MERCHANT_ID,
            );
    ...
    describe('submitForReview (DRAFT/REJECTED → PENDING)', () => {
        ...
        it('should reject submit without room types', async () => {
            ...
            const saved = await ds.getRepository(Hotel).save(hotel);
            await expect(service.submitForReview(saved.id, MERCHANT_ID)).rejects.toThrow(ForbiddenException);
        });
        ...
        it('should reject submit without openingDate', async () => {
            ...
            const saved = await ds.getRepository(Hotel).save(hotel);
            await ds.getRepository(RoomType).save({ name: '房间', price: 100, maxGuests: 2, hotelId: saved.id } as any);
            await expect(service.submitForReview(saved.id, MERCHANT_ID)).rejects.toThrow(ForbiddenException);
        });
    });
    ...
    describe('submitForReview (illegal transitions)', () => {
        it.each([
            ...
            HotelStatus.OFFLINE,
        ])('should reject submit from %s status', async (status) => {
            const hotel = await createHotelInStatus(status);
            await expect(service.submitForReview(hotel.id, MERCHANT_ID)).rejects.toThrow(ForbiddenException);
        });
    });
    ...
    describe('approve (illegal transitions)', () => {
        it.each([
            ...
            const hotel = await createHotelInStatus(status);
            await expect(service.approve(hotel.id)).rejects.toThrow(ForbiddenException);
        });
    });
    ...
    describe('reject (illegal transitions)', () => {
        it.each([
            ...
            const hotel = await createHotelInStatus(status);
            await expect(service.reject(hotel.id, '理由')).rejects.toThrow(ForbiddenException);
        });
    });
    ...
    describe('offline (illegal transitions)', () => {
        it.each([
            ...
            const hotel = await createHotelInStatus(status);
            await expect(service.offline(hotel.id)).rejects.toThrow(ForbiddenException);
        });
    });
    ...
    describe('online (illegal transitions)', () => {
        it.each([
            ...
            const hotel = await createHotelInStatus(status);
            await expect(service.online(hotel.id)).rejects.toThrow(ForbiddenException);
        });
    });
    ...
    describe('update (auto re-review)', () => {
        it('should change APPROVED → PENDING on update', async () => {
            const hotel = await createHotelInStatus(HotelStatus.APPROVED);
            const result = await service.update(hotel.id, { nameCn: '修改后的酒店名' } as any, MERCHANT_ID);
            expect(result.status).toBe(HotelStatus.PENDING);
            // 应该通知管理员
            expect(notificationsSent.some((n) => n.type === 'hotel_submitted' && n.targetRole === 'admin')).toBe(true);
        });

        it('should change OFFLINE → PENDING on update', async () => {
            const hotel = await createHotelInStatus(HotelStatus.OFFLINE);
            const result = await service.update(hotel.id, { nameCn: '修改名称' } as any, MERCHANT_ID);
            expect(result.status).toBe(HotelStatus.PENDING);
        });

        it('should keep DRAFT status on update', async () => {
            const hotel = await createHotelInStatus(HotelStatus.DRAFT);
            const result = await service.update(hotel.id, { nameCn: '草稿修改' } as any, MERCHANT_ID);
    ...
    describe('ownership isolation', () => {
        it('should reject update by non-owner', async () => {
            ...
            await expect(
                service.update(hotel.id, { nameCn: '被其他人修改' } as any, OTHER_MERCHANT_ID),
            ).rejects.toThrow(ForbiddenException);
        });
        ...
        it('should reject delete by non-owner', async () => {
            const hotel = await createHotelInStatus(HotelStatus.DRAFT, MERCHANT_ID);
            await expect(service.remove(hotel.id, OTHER_MERCHANT_ID)).rejects.toThrow(ForbiddenException);
        });
    });
    ...
    describe('remove', () => {
        it('should allow deleting DRAFT hotels', async () => {
            const hotel = await createHotelInStatus(HotelStatus.DRAFT);
            await expect(service.remove(hotel.id, MERCHANT_ID)).resolves.toBeUndefined();
        });
        ...
        it.each([
            ...
            const hotel = await createHotelInStatus(status);
            await expect(service.remove(hotel.id, MERCHANT_ID)).rejects.toThrow(ForbiddenException);
        });
    });
    ...
    describe('public queries', () => {
        ...
        it('findOneApproved should throw for non-APPROVED hotel', async () => {
            const hotel = await createHotelInStatus(HotelStatus.DRAFT);
            await expect(service.findOneApproved(hotel.id)).rejects.toThrow(NotFoundException);
        });
        ...
        it('findApprovedHotels should only return APPROVED hotels', async () => {
            ...
            // 所有结果均为 APPROVED
            expect(result.data.every((h) => h.status === HotelStatus.APPROVED)).toBe(true);
        });
    });
    ...
    describe('notification integration', () => {
        it('should send admin broadcast on submit', async () => {
            ...
            const notification = notificationsSent.find((n) => n.type === 'hotel_submitted');
        ...
        it('should send precise user notification on approve', async () => {
            ...
            const notification = notificationsSent.find((n) => n.type === 'hotel_approved');
        ...
        it('should send precise user notification on reject', async () => {
            ...
            const notification = notificationsSent.find((n) => n.type === 'hotel_rejected');
            expect(notification.targetUserId).toBe(MERCHANT_ID);
    ...
    describe('SSE price events', () => {
        it('should emit hotel_online on approve', async () => {
            const hotel = await createHotelInStatus(HotelStatus.PENDING);
            await service.approve(hotel.id);
            expect(priceEventsEmitted.some((e) => e.kind === 'hotel_online' && e.hotelId === hotel.id)).toBe(true);
        });

        it('should emit hotel_offline on offline', async () => {
            const hotel = await createHotelInStatus(HotelStatus.APPROVED);
            await service.offline(hotel.id);
            expect(priceEventsEmitted.some((e) => e.kind === 'hotel_offline' && e.hotelId === hotel.id)).toBe(true);
        });
