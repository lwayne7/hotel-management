...
async function seed() {
  ...
  if (createdUsers > 0) {
    console.log(
      `   ✓ 新增了 ${createdUsers} 个用户（总模板 ${SEED_USERS.length}）`,
    );
  } else {
  ...
  if (existingHotels === 0) {
    ...
    await dataSource.transaction(async (manager) => {
      ...
      for (let i = 0; i < FEATURED_HOTELS.length; i++) {
        ...
        const cityIndex = ALL_CITIES.findIndex((c) =>
          FEATURED_HOTELS[i].nameCn.startsWith(c),
        );
        ...
        for (let j = 0; j < FEATURED_HOTELS[i].roomTypes.length; j++) {
          ...
          allRoomTypes.push({
            ...
            imageUrl:
              rt.imageUrl ||
              getRoomImageByType(rt.name, hotelId, j, safeCityIndex),
            hotelId,
          });
        }
      }
      ...
      for (let i = 0; i < FEATURED_HOTELS.length; i++) {
        ...
        const imageCount = Math.min(
          4,
          Math.max(2, FEATURED_HOTELS[i].images.length),
        );
        const imagesToSave = generateHotelImages(
          hotelId,
          safeCityIndex,
          imageCount,
        );
  ...
  if (existingInventories === 0) {
    ...
    console.log(
      `   ✓ 初始化库存记录 ${inventoryRows.length} 条（房型数 ${roomTypes.length}）`,
    );
  } else {
    console.log('\n⚠️  数据库已有库存数据，跳过库存初始化');
  }
