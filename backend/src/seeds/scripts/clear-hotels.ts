...
async function clearHotels() {
    ...
    const deletedImages = await hotelImageRepository.createQueryBuilder().delete().execute();
    const deletedRoomTypes = await roomTypeRepository.createQueryBuilder().delete().execute();
    const deletedHotels = await hotelRepository.createQueryBuilder().delete().execute();

    const imgCount = typeof deletedImages.affected === 'number' ? deletedImages.affected : 0;
    const roomCount = typeof deletedRoomTypes.affected === 'number' ? deletedRoomTypes.affected : 0;
    const hotelCount = typeof deletedHotels.affected === 'number' ? deletedHotels.affected : 0;
