...
async function checkUrl(
  url: string,
): Promise<{ ok: boolean; status: number | 'ERR'; error?: string }> {
  try {
...
async function main() {
  ...
  const concurrency = Math.max(
    1,
    Math.min(32, Number(process.env.CONCURRENCY ?? 12) || 12),
  );

  console.log(
    `🔎 校验图片 URL... (unique=${urls.length}, concurrency=${concurrency})`,
  );
  ...
  if (bad.length > 0) {
    ...
    if (bad.length > 30)
      console.error(`... (还有 ${bad.length - 30} 个未展示)`);
    process.exit(1);
  }
