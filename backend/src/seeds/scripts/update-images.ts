...
async function updateImages() {
  ...
  for (const rt of roomTypes) {
    ...
    const cityIndex = hotel ? getCityIndexFromName(hotel.nameCn) : 0;
    const newUrl = getRoomImageByType(
      rt.name,
      rt.hotelId,
      roomIndex,
      cityIndex,
    ...
    else if (rt.name.includes('双床') || rt.name.includes('标准间'))
      stats['双床房']++;
    else if (rt.name.includes('家庭') || rt.name.includes('亲子'))
      stats['家庭房']++;
    else if (rt.name.includes('大床') || rt.name.includes('豪华'))
      stats['大床房']++;
    else stats['标准间']++;
  }
