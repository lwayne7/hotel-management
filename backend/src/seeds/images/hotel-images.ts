 ...
 */
export function getExteriorImage(hotelId: number, _cityIndex: number = 0, offset: number = 0): string {
 ...
 * 使用强哈希确保不同酒店获得不同图片
 */
export function getLobbyImage(hotelId: number, cityIndex: number = 0, offset: number = 0): string {
 ...
 * 使用强哈希确保不同酒店获得不同图片
 */
export function getPoolImage(hotelId: number, cityIndex: number = 0, offset: number = 0): string {
...
export function generateHotelImages(
    ...
    const blockCapacity = exteriorTotal > 0 ? Math.floor(exteriorTotal / blockSize) : 0;
    ...
    if (targetCount >= 2) pushWithRetry((i) => getLobbyImage(hotelId, cityIndex, i), '酒店大堂', 6);
    if (targetCount >= 3) pushWithRetry((i) => getPoolImage(hotelId, cityIndex, i), '泳池设施', 6);
    if (targetCount >= 4) pushWithRetry((i) => getExteriorImage(hotelId, cityIndex, 400 + i * 400), '酒店外观', 6);

    // 3. 如果因为去重导致数量不足，则继续用不同 offset 的外观图补齐，
    //    直到达到目标数量或外观图库已被当前酒店用完
    let extraOffset = 800;
    while (result.length < targetCount && used.size < HOTEL_EXTERIOR_IMAGES.length) {
