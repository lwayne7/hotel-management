...
function randomSubset<T>(rng: () => number, arr: T[], min: number, max: number): T[] {
...
function generateHotelName(rng: () => number, city: string, starRating: number, index: number): { nameCn: string; nameEn: string } {
    ...
    const frontendBrandsForStar = FRONTEND_BRANDS.filter(brand => {
        // 根据品牌判断星级范围
        const luxuryBrands = ['洲际', '凯悦', '希尔顿', '喜来登', '万豪', '香格里拉', '四季'];
    ...
    // 如果前端品牌列表中有匹配的，优先使用
    const availableBrands = frontendBrandsForStar.length > 0 ? frontendBrandsForStar : brands;
    const brand = availableBrands[Math.floor(rng() * availableBrands.length)];
    
    const suffixIndex = Math.floor(index / availableBrands.length) % HOTEL_SUFFIXES.length;
    const suffix = HOTEL_SUFFIXES[suffixIndex];
    // 为避免重名，在名称后加上编号（当索引较大时）
    const suffix2 = index >= availableBrands.length * HOTEL_SUFFIXES.length ? `${Math.floor(index / (availableBrands.length * HOTEL_SUFFIXES.length)) + 1}号店` : '';
 ...
 */
function generateFacilities(seed: number, starRating: number, cityIndex: number): string[] {
    ...
    switch (primaryTag) {
        case '亲子':
            facilities.push(...randomSubset(rng, FACILITY_GROUPS.family.tags.filter(t => t !== primaryTag), 1, 3));
            break;
        case '豪华':
            facilities.push(...randomSubset(rng, FACILITY_GROUPS.luxury.tags.filter(t => t !== primaryTag), 1, 3));
            break;
        case '免费停车场':
            facilities.push(...randomSubset(rng, FACILITY_GROUPS.parking.tags.filter(t => t !== primaryTag), 0, 2));
            break;
        case '含早餐':
            facilities.push(...randomSubset(rng, FACILITY_GROUPS.breakfast.tags.filter(t => t !== primaryTag), 0, 2));
            break;
        case '健身房':
            facilities.push(...randomSubset(rng, FACILITY_GROUPS.fitness.tags.filter(t => t !== primaryTag), 1, 3));
            break;
        case '近地铁':
            facilities.push(...randomSubset(rng, FACILITY_GROUPS.metro?.tags || ['近地铁'], 0, 2));
            break;
        case '游泳池':
            facilities.push(...randomSubset(rng, FACILITY_GROUPS.pool.tags.filter(t => t !== primaryTag), 0, 2));
            break;
    }

    // 根据星级添加额外设施（使用确定性随机）
    if (starRating >= 4) {
        if (rng() > 0.3) facilities.push(...randomSubset(rng, FACILITY_GROUPS.pool.tags, 1, 2));
        if (rng() > 0.3) facilities.push(...randomSubset(rng, FACILITY_GROUPS.spa.tags, 1, 2));
    }
    if (starRating >= 3) {
...
function generateRoomTypes(
    ...
    // 先选择基础房型配置
    let selectedConfigs = randomSubset(rng, availableConfigs, roomCount, roomCount);
    
    // 如果酒店有亲子标签，确保至少有一个家庭房或亲子主题房
    const hasFamilyTag = facilities.some(f => f.includes('亲子'));
    if (hasFamilyTag) {
        const familyConfigs = availableConfigs.filter(c => 
            c.name.includes('家庭') || c.name.includes('亲子')
        );
        if (familyConfigs.length > 0) {
            const hasFamilyRoom = selectedConfigs.some(c => 
                c.name.includes('家庭') || c.name.includes('亲子')
            );
            if (!hasFamilyRoom) {
                // 替换第一个为家庭房
    ...
    const hasTwinBed = selectedConfigs.some(c => c.name.includes('双床') || c.name.includes('标准'));
    if (!hasBigBed && !hasTwinBed) {
        // 如果没有大床或双床，添加一个
        const bedConfigs = availableConfigs.filter(c => 
            c.name.includes('大床') || c.name.includes('双床') || c.name.includes('标准')
        );
    ...
    return selectedConfigs.map((config, roomIndex) => {
        ...
        const imageUrl = getRoomImageByType(config.name, hotelIndex, roomIndex, cityIndex);
...
export function generateHotels(options: GenerateHotelsOptions): SeedHotel[] {
    ...
    for (let c = 0; c < ALL_CITIES.length; c++) {
        ...
        for (let star = 1; star <= 5; star++) {
            const countForStar = Math.floor(starsPerCity / 5) + (star <= (starsPerCity % 5) ? 1 : 0);
    ...
    for (let i = 0; i < count; i++) {
        ...
        const starRating = (cityStars[cityHotelIndex % cityStars.length] || ((cityHotelIndex % 5) + 1)) as 1 | 2 | 3 | 4 | 5;
        ...
        const roomTypes = generateRoomTypes(seed, index, starRating, cityIndex, facilities);
