...
> 面试可讲：如何从一条告警（例如「订单接口 P95 > 2s」）追到具体请求 / SQL：告警 → `/metrics` → 日志中的 `requestId` → 结合业务字段（`orderId/roomTypeId`）做问题归因。
...
> 面试可讲：如何在「本地开发 / 预发布 / 线上」之间切换配置；如何通过 `/healthz` + `/metrics` 快速判断是「依赖挂了」还是「某个接口慢了」。
