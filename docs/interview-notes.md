...
- **Trade-off**：
  ...
  - 热点房型在高并发下会有锁竞争，但在当前项目体量下是可以接受的；后续可进一步拆分热点行来优化
...
- **设计要点**：
  - 不同实时方案的优缺点：实现复杂度 / 兼容性 / 连接数量等
  - “先用简单方案解决 80% 问题，再用适配层兼容剩下 20%”的思路
...
> 核心理念：不仅仅是”写了测试”，而是围绕业务不变量设计测试，并把这些检查挂到 CI 上。
...
项目围绕”业务问题 → 方案 → 效果 → 权衡”展开设计，而不仅仅是技术栈的使用。
